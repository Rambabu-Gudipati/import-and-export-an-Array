...
app.get("/movies/:movieId/", async (request, response) => {
  ...
  response.send(convertMovieDbObjectToResponsiveObject(movie));
});
...
app.get("/directors/", async (request, response) => {
  ...
  response.send(
    directorsArray.map((eachDirector) =>
      convertDirectorDbObjectToResponsiveObject(eachDirector)
    )
  );
});
...
app.get("/directors/:directorId/movies/", async (request, response) => {
  ...
  response.send(
    moviesArray.map((eachMovie) => ({ movie_name: eachMovie.movie_name }))
  );
});
